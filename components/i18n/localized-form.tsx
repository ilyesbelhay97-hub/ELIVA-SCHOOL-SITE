...
export function LocalizedRegistrationForm({ locale, courseSlug }: { locale: Locale; courseSlug?: string }) {
  ...
  const beginRegistration = () => { if (started) return; setStarted(true); trackMetaEvent("InitiateCheckout", { course_id: values.course || undefined, course_name: courseNames[values.course]?.[locale], study_mode: values.mode || undefined, locale }); };
  const update = (key: string, value: string | boolean) => { beginRegistration(); setValues((current) => ({ ...current, [key]: value })); };
  async function submit(event: FormEvent) {
    ...
    const payload: RegistrationPayload = { fullName: values.name, phone: values.phone, email: values.email || undefined, wilaya: values.wilaya, formation: values.course, courseName: courseNames[values.course]?.[locale] ?? values.course, studyMode: values.mode, message: values.message || undefined, consent: true, status: "new", sourcePage: typeof window !== "undefined" ? window.location.pathname : undefined, ...getAttribution() };
    try { const result = await submitRegistration(payload); if (!result.ok) throw new Error(); trackMetaEvent("Lead", { content_name: payload.courseName, content_category: "formation", course_id: payload.formation, study_mode: payload.studyMode, locale }); setSent(true); } catch { setError(s.error); }
  }
